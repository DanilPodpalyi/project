...
`resume_evaluator.vacancy_search` берёт поле `target_position` из структурированного JSON
...
```powershell
python -m resume_evaluator.resume_extractor examples/resume.txt output/resume.json
```

Затем выполните поиск по всем регионам:

```powershell
python -m resume_evaluator.vacancy_search output/resume.json output/vacancies_found.json
```
