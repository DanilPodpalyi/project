...
- прозрачная оценка соответствия кандидата вакансии по шкале 0–100;
- краткая и подробная рекомендации кандидату;
- независимая оценка качества резюме без вакансии;
- поиск актуальных вакансий через публичный API hh.ru;
- автоматическое построение синтетической эталонной вакансии по 20+ публичным
...
├── docs/                             # Локальная дополнительная документация
├── data/
│   ├── reference/                    # Локальные справочники, исключены из Git
│   │   ├── industries.json           # 35 отраслей
│   │   └── professions.json          # Справочник профессий
...
вакансии через API hh.ru. На момент минимальной приёмки модуль не работает,
...
Частоты требований и ссылки на источники сохраняются в итоговом JSON.
...
Для `gemma4:12b` требуется заметный объём оперативной или видеопамяти. Если
модель не помещается в доступные ресурсы, имя модели нужно согласованно изменить
в модулях, использующих Ollama.
...
Проверка Python-зависимостей и схем без загрузки модели:

```powershell
python -m resume_evaluator.system_check
```

Контрольный запрос к реальной модели:
